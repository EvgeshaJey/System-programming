...
int main() {
    ...
    // Ввод ключа
    printf("Enter key for shared memory (server 1): ");
    scanf("%d", &key);
    ...
    // Создание семафора
    sem = sem_open("/shared_semaphore", O_CREAT, 0666, 1);
    if (sem == SEM_FAILED) {
        perror("sem_open");
        exit(1);
    }
    ...
    while (1) {
        ...
        snprintf(shm, SHM_SIZE, "Server 1: %s", message);
